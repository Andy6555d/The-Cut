...
export function HomeDashboard(){const[ready,setReady]=useState(false);useEffect(()=>setReady(true),[]);const stats=useMemo(()=>ready?PRACTICE_KEYS.map(x=>getPracticeStats(x.key,x.dir)):[],[ready]);const totalPractice=stats.reduce((s,x)=>s+x.attemptCount,0);const trained=stats.filter(x=>x.attemptCount>0).length;
 ...
 useEffect(() => {
   fetch("/api/player/streak").then(r => r.json()).then(d => setStreak(d.currentStreak ?? 0)).catch(() => {});
 }, []);
 const pressure=localWeeklyPressure();
 return <main className="home-shell">{!howToSeen ? <HowToPlayPopup onDismiss={() => setHowToSeen(true)} /> : <HomeNamePopup/>}<div className="ambient-orb orb-one"/><div className="ambient-orb orb-two"/><section className="home-hero"><div className="home-eyebrow"><span className="live-dot"/> WORLD DAILY · ONE SHOT · RESETS AT YOUR MIDNIGHT</div>{streak > 0 && <div className="home-streak-badge">🔥 <strong>{streak}</strong> day streak</div>}<h1 className="brand-lockup"><span>THE</span><strong>CUT</strong></h1><p className="home-tagline">TRAIN ANYTIME. COMPETE ONCE.<br/><b>HOW FAR CAN YOU GO?</b></p><div className="world-ring" aria-hidden="true"><div className="world-ring-inner">✦</div></div><div className="weekly-pressure-banner"><strong>{pressure.day} · {pressure.tier}</strong><span>{pressure.note} Resets Sunday at your midnight.</span></div><div className="countdown-card"><span>YOUR NEXT CUT IN</span><Countdown/></div><Link href="/daily" className="hero-cta">PLAY TODAY&apos;S CUT <span>→</span></Link><div className="home-mode-row"><Link href="/practice" className="practice-cta">◎ PRACTICE</Link><Link href="/levels" className="practice-cta levels-cta">▲ LEVELS</Link><Link href="/practice/quick" className="practice-cta quick-cta">⚄ QUICK</Link></div></section>
